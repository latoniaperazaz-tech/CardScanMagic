...
def detect_pips(image: np.ndarray) -> list[dict[str, Any]]:
    ...
    # Keep the color channels separate while extracting components. On a dim
    # card with a white masking background, the gray card stock may enter the
    # dark mask. Merging first would attach a red pip to that oversized region.
    for component_mask in (red_mask, dark_mask):
        count, labels, stats, _ = cv2.connectedComponentsWithStats(
            component_mask,
            connectivity=8,
        )
        for label in range(1, count):
            x, y, component_width, component_height, pixel_area = [
            ...
            for section_x, section_y, section_width, section_height in sections:
                ...
                section_mask = np.zeros_like(red_mask)
